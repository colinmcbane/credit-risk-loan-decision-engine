...
def build_mean_pd_comparison(df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_hline(
        ...
        annotation_text="Unfavorable Threshold",
        annotation_position="top right",
    )
...
def layout():
    ...
    return html.Div([
        ...
        # ── Section header ────────────────────────────────────────────────
        html.H3("Synthetic Test Analysis", className="mb-1"),
        html.P(
        ...
        dbc.Row(build_source_summary_cards(df), className="mb-4 g-3"),

        # ── Charts row 1 ─────────────────────────────────────────────────
        dbc.Row([
            dbc.Col(dcc.Graph(figure=build_decision_comparison(df)), width=6),
            dbc.Col(dcc.Graph(figure=build_mean_pd_comparison(df)), width=6),
        ], className="mb-4"),

        # ── Charts row 2 ─────────────────────────────────────────────────
        dbc.Row([
            dbc.Col(dcc.Graph(figure=build_prob_distribution(df)), width=6),
            dbc.Col([
                html.H5("Risk Tier Accuracy", className="mb-3"),
                html.P(
